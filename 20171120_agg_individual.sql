...
CREATE TABLE            agg.indiv_survey_resp_dt_temp
    ...
    SELECT
        individual_id
        , max(cast(answer_date as timestamp)) answer_date
        , min(cast(answer_date as timestamp)) min_answer_date
...
CREATE TABLE            agg.indiv_off_canc_temp
    ...
    WITH cte_print_canc AS (
        SELECT
            ah.hash_account_id
            , ah.action_id
            , oca.cancel_reason_code
        FROM prod.action_header ah
            INNER JOIN prod.print_cancel oca
                ON ah.hash_action_id = oca.hash_action_id
    )
    SELECT
        t1.individual_id
    ...
    FROM prod.agg_print_order t1
        LEFT JOIN cte_print_canc oca
            ON oca.hash_account_id = t1.hash_account_id
               AND oca.action_id = t1.ord_id
    WHERE 1 = 1
...
( SELECT
      ...
      , CAST(NULL AS TIMESTAMP) AS max_pldg_dt
), cte_fr_lt_pledge AS
( SELECT
        CAST(NULL AS BIGINT)    AS individual_id
      , CAST(NULL AS TIMESTAMP) AS max_pldg_dt
...
SELECT
    ...
    , MAX(em.update_date)              AS PROSPECT_EMAIL_MATCH_DT
            ...
            THEN 'Y'
                           ...
                           GROUP BY individual_id
                                 ...
                                 --SD 20171017: Outside List are deprecated
                                   cast(NULL AS BIGINT)      AS individual_id
                                 , cast(NULL AS TIMESTAMP)   AS ol_match_recent_dt
                                 , 0                         AS ol_match_cnt
                                 , 0                            ol_match_cnt_6m
...
CREATE TABLE            prod.agg_individual
    ...
    AS
        SELECT
            ...
            , ind_ltd_amt
            , comp_flg
            , cast(ind_fst_rel_dt as TIMESTAMP)                                                       AS ind_fst_rel_dt
            , ind_lst_act_dt
            , ind_actv_cnt
            , ind_relship_cnt
            ...
            , ipad_enabled_dt                                                                         AS ipad_enabled_dt
