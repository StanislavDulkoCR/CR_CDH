...
create table mt_ind_email_flag_temp
as
select a.individual_id,
       max(CASE WHEN a.NEWS_IND_ACTV_FLG = 'Y' and e.email_type_cd in ('N','I') AND (nvl(e.valid_flag,'X') <> 'N' and nvl(e.src_valid_flag,'X') <> 'N' and nvl(e.src_delv_ind,'X') <> '1')
                 THEN 'Y' ELSE 'N' END) as news_ind_actv_flg
       from 
               (SELECT individual_id,
                        MAX(CASE WHEN scp_cd LIKE 'NEW%' AND SCP_CD NOT IN ('NEWHHEART', 'NEWHCHILDTEEN', 'NEWHDIABETES', 'NEWHWOMEN', 'NEWHAFTER60') AND auth_flg = 'Y' THEN 'Y'
                                 ELSE NULL END) AS NEWS_IND_ACTV_FLG
                  FROM mt_authorization
               ...
               mt_email e
where a.individual_id = e.individual_id (+)
group by a.individual_id;


create table mt_olo_xog_temp
PARALLEL (DEGREE 8)
as
...
from warehouse.individual_xographic xo,
     mt_offline_ord olo
where xo.individual_id = olo.individual_id (+)
group by xo.individual_id;

create table mt_oli_aps_temp
PARALLEL (DEGREE 8)
as
select xo.individual_id, 
...
create table mt_indiv_surv
PARALLEL (DEGREE 8)
as
select individual_id, max(answer_date) answer_date, min(answer_date) min_answer_date
from warehouse.survey_response_fact
where substr(individual_id,1,1) = 1
group by individual_id;

create table mt_indiv_page_temp
PARALLEL (DEGREE 8)
 as
select individual_id, max(hit_time) hit_time, min(hit_time) min_hit_time
from warehouse.web_page
where event_list_id = 233
group by individual_id;

create table mt_indiv_page_2_temp
PARALLEL (DEGREE 8)
 as
select individual_id, CASE WHEN page.hit_time > sysdate - 365 THEN 1 ELSE 0 END as page_view_within_year 
from mt_indiv_page_temp page; 
...
CREATE TABLE mt_individual_ibx
  PCTUSED 99
  COMPRESS
  ...
  FROM (
        SELECT xr.individual_id,
         ...
         substr(LEAST(NVL(ofo.ofo_fst_prod_cd,                                '99991231'||'99')
               ...
               + DECODE(bs.bk_actv_flg,'Y',1,0)
               + CASE WHEN surv.answer_date >= ADD_MONTHS(SYSDATE,-12) THEN 1 ELSE 0 END
               + CASE WHEN bb.bnb_last_sales_dt > ADD_MONTHS(SYSDATE,-12) THEN 1 ELSE 0 END
               + NVL(page2.page_view_within_year,0) 
               + CASE WHEN nvl(neflag.NEWS_IND_ACTV_FLG,'N') = 'Y' THEN 1 ELSE 0 END
               + CASE WHEN adv_mt.adv_active_flg = 'Y' THEN 1 ELSE 0 END AS ind_actv_cnt, 
               ...
               + CASE WHEN ofs.individual_id IS NULL AND (ix.dc_offline_first_dt IS NOT NULL OR (ix.dc_corp_first_dt IS NOT NULL AND NVL(oloxog.dc_corp_first_ind,0) = 0)) THEN 1 ELSE 0 END
               ...
               + CASE WHEN nvl(ols.nc_rpts_flg,'N') = 'N' AND nvl(ols.uc_rpts_flg,'N') = 'N' AND ix.dc_last_aps_ord_dt IS NOT NULL THEN 1 ELSE 0 END
               + CASE WHEN ols.individual_id IS NULL AND ix.dc_online_first_dt IS NOT NULL AND NVL(olixog.dc_online_fst_aps_ind,0) = 0 THEN 1 ELSE 0 END
               + CASE WHEN fs.individual_id IS NOT NULL OR abqd.individual_id IS NOT NULL THEN 1 ELSE 0 END
               + CASE WHEN fs.individual_id IS NULL AND abqd.individual_id IS NULL AND ix.dc_fund_first_dt IS NOT NULL THEN 1 ELSE 0 END
               + CASE WHEN abq.abq_ltd_rsp_cnt > 0 OR surv.individual_id IS NOT NULL THEN 1 ELSE 0 END
               + CASE WHEN bb.bnb_tot_sale_cnt > 0 THEN 1 ELSE 0 END
               + CASE WHEN a.news_auth_cnt > 0 THEN 1 ELSE 0 END
               + CASE WHEN page.individual_id IS NOT NULL THEN 1 ELSE 0 END
               + CASE WHEN eread.individual_id IS NOT NULL THEN 1 ELSE 0 END
               ...
               + NVL(oli.oli_aps_prod_cnt,0) + NVL(bi.bi_cnt,0) + NVL2(ix.dc_cdb_last_ord_dt,1,0) + NVL(eread.ereader_cnt,0)
               + CASE WHEN ix.dc_online_first_dt IS NOT NULL AND NVL(olixog.dc_online_fst_crt_ind,0) = 0 AND NVL(olixog.dc_online_fst_aps_ind,0) = 0 THEN 1 ELSE 0 END
               + CASE WHEN ((ix.dc_offline_first_dt IS NOT NULL OR (ix.dc_corp_first_dt IS NOT NULL AND NVL(oloxog.dc_corp_first_ind,0) = 0)) AND 
                         (NVL(oloxog.dc_offline_fst_crt_ind,0) = 0 AND NVL(oloxog.dc_corp_fst_crt_ind,0) = 0)) THEN 1 ELSE 0 END
               + CASE WHEN ix.dc_last_aps_ord_dt IS NOT NULL AND NVL(oliaps.dc_last_aps_crt_ind,0) = 0 THEN 1 ELSE 0 END AS prod_cnt,  
               ix.ad_apl_keycode,
          ...
          FROM ind_xref xr,
               warehouse.individual_xographic ix,
               mt_abq_summary abq,
               mt_fr_summary fs,
               mt_offline_summary ofs,
               mt_online_summary ols,
               warehouse.advocacy adv,
               mt_advocacy adv_mt,
               (SELECT individual_id,
                       coa_date
                  FROM mt_name_address
               ...
               (SELECT DISTINCT secd_indiv_id
                  FROM warehouse.fundraising_combination
               ) frc2,
               (SELECT individual_id,
               ...
               (SELECT er.individual_id,
                  ...
                  FROM warehouse.external_ref er,
                       warehouse.offline_account_arch t1
               ...
               (SELECT t1.individual_id,
                  ...
                  FROM mt_offline_ord t1,
                       warehouse.offline_cancel_act oca
               ...
               (SELECT individual_id,
                  ...
                  FROM warehouse.outside_list
                 GROUP BY individual_id) ol,
               mt_alliant al,
               mt_individual_ibx ibx ,
                (select  min(eua.create_date) as min_create_date, er.individual_id
              from warehouse.erights_user_association eua, 
              ...
              group by er.individual_id                                      
                ...
                (select b.individual_id,
                    ...
                    from mt_build_and_buy b
                    group by b.individual_id
                     ) bb,
                     mt_pros_email_ind_temp pem,
               warehouse.individual_address ia,
               (SELECT er.individual_id,
                       MAX(il.pub_dt) AS mx_cds_pub_dt
                  FROM warehouse.external_ref er
                       INNER JOIN warehouse.offline_account oa ON oa.acct_id = er.internal_key
                       INNER JOIN warehouse.issue_lkp il ON il.mag_cd = oa.mag_cd AND il.iss_num = oa.lst_iss_num
                 WHERE er.data_source IN ('CDS')
                 GROUP BY er.individual_id
               ) erm,
                (SELECT mos1.individual_id
                  FROM mt_offline_summary mos1,
                       (SELECT individual_id
                          FROM mt_offline_summary
                 ...
                 (select individual_id, max(report_start_date) report_start_date, min(report_start_date) min_rpt_start_date, count(*) ereader_cnt
                   ...
                   group by individual_id) eread,
                    mt_indiv_surv surv,
                    mt_indiv_page_temp page,
                    mt_indiv_page_2_temp page2,
                    mt_oli_xog_temp olixog,
                    mt_oli_aps_temp oliaps,
                    mt_olo_xog_temp oloxog,
                    mt_ind_email_flag_temp neflag
         WHERE xr.individual_id = ix.individual_id
           ...
           AND xr.individual_id = page2.individual_id (+)
           ...
           AND xr.individual_id = neflag.individual_id (+)
       )
 GROUP BY individual_id,
          ind_urn,
          hh_id,
