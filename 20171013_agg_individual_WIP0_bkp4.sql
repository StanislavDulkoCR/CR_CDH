    ...
    AS
        SELECT
            ...
            , DECODE(fin.ofs_cr_actv_flg, 'Y', 1, 0) + DECODE(fin.ofs_cr_sub_dnr_flg, 'Y', 1, 0) + DECODE(fin.ofs_cr_non_sub_dnr_flg, 'Y', 1, 0)
              ...
              + DECODE(fin.ofs_ma_actv_flg, 'Y', 1, 0) + DECODE(fin.ofs_ma_sub_dnr_flg, 'Y', 1, 0) + DECODE(fin.ofs_ma_non_sub_dnr_flg, 'Y', 1, 0)
              + DECODE(fin.ofs_shm_actv_flg, 'Y', 1, 0) + DECODE(fin.ofs_shm_sub_dnr_flg, 'Y', 1, 0) + DECODE(fin.ofs_shm_non_sub_dnr_flg, 'Y', 1, 0)
              + DECODE(fin.ols_cro_actv_flg, 'Y', 1, 0) + DECODE(fin.ols_cro_sub_dnr_flg, 'Y', 1, 0) + DECODE(fin.ols_cro_non_sub_dnr_flg, 'Y', 1, 0)
              + DECODE(fin.ols_carp_actv_flg, 'Y', 1, 0)
              /*+ CASE WHEN fin.ols_nc_rpts_lst_ord_dt > ADD_MONTHS(SYSDATE, -12)
            THEN 1
                ELSE 0 END*/
              /*+ CASE WHEN fin.ols_uc_rpts_lst_ord_dt > ADD_MONTHS(SYSDATE, -12)
            THEN 1
                ELSE 0 END*/
              + CASE WHEN fin.fs_fr_lst_don_dt > ADD_MONTHS(SYSDATE, -12)
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.abq_abq_mbr_flg = 'Y'
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.a_auth_flg_cnt > 0
            THEN 1
                ELSE 0 END
              + DECODE(fin.bs_bk_actv_flg, 'Y', 1, 0) + DECODE(fin.a_news_car_watch_flg, 'Y', 1, 0) + DECODE(fin.a_news_best_buy_drug_flg, 'Y', 1, 0)
              + DECODE(fin.a_news_safety_alert_flg, 'Y', 1, 0) + DECODE(fin.a_news_hlth_alert_flg, 'Y', 1, 0) + DECODE(fin.a_news_cro_whats_flg, 'Y', 1, 0)
              + DECODE(fin.a_news_green_choice_flg, 'Y', 1, 0)
              + CASE WHEN fin.adv_mt_adv_active_flg = 'Y' OR fin.adv_mt_adv_volunteer_flg = 'Y'
            THEN 1
            ...
            , DECODE(NVL(fin.ofs_cr_flg, 'N'), 'N', 0, DECODE(fin.ofs_cr_fst_dnr_dt, NULL, 1, 2))
              ...
              + CASE WHEN fin.ofs_individual_id IS NULL AND (fin.ix_dc_offline_first_dt IS NOT NULL OR fin.ix_dc_corp_first_dt IS NOT NULL)
            THEN 1
                ELSE 0 END
            ...
            THEN 1
              ...
              + CASE WHEN /*fin.ols_nc_rpts_flg = 'N' AND*/ fin.ols_uc_rpts_flg = 'N' AND fin.ix_dc_last_aps_ord_dt IS NOT NULL
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.ols_individual_id IS NULL AND fin.ix_dc_online_first_dt IS NOT NULL
            THEN 1
                ELSE 0 END
              + NVL2(fin.fs_individual_id, 1, 0)
              + CASE WHEN fin.fs_individual_id IS NULL AND fin.ix_dc_fund_first_dt IS NOT NULL
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.abq_abq_ltd_mbr_cnt > 0
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.a_auth_cnt > 0
            THEN 1
                ELSE 0 END
            ...
            , NVL(fin.oli_oli_cro_prod_cnt, 0) + NVL(fin.oli_oli_carp_prod_cnt, 0) + NVL(fin.oli_oli_crmg_prod_cnt, 0) + NVL(fin.brks_cr_prod_cnt, 0) + NVL(fin.de_de_cr_prod_cnt, 0)
              ...
              + NVL(fin.oli_oli_aps_prod_cnt, 0) + NVL(fin.bi_bi_cnt, 0) + NVL2(fin.ix_dc_cdb_last_ord_dt, 1, 0)
              + CASE WHEN fin.ix_dc_online_first_dt IS NOT NULL AND nvl(fin.oli_oli_cro_prod_cnta, 0) = 0
            THEN 1
                ELSE 0 END
              + CASE WHEN (fin.ix_dc_offline_first_dt IS NOT NULL OR fin.ix_dc_corp_first_dt IS NOT NULL) AND nvl(fin.brks_cr_prod_cnta, 0) = 0
            THEN 1
                ELSE 0 END
              + CASE WHEN fin.ix_dc_last_aps_ord_dt IS NOT NULL
            THEN 1
