create table cr_temp.resp_ind_xref_ph
AS
...
create table cr_temp.resp_ph 
AS
    SELECT t1.*
    FROM mt_promotion t1
      , cr_temp.resp_ind_xref_ph t2
    WHERE t1.individual_id = t2.individual_id
        AND t1.contact_type = 'P';

create table cr_temp.resp_mt_online_ord 
AS
    SELECT t1.* FROM mt_online_ord t1, cr_temp.resp_ind_xref_ph t2 WHERE t1.individual_id = t2.individual_id;

create table cr_temp.resp_mt_online_item 
AS
    SELECT t1.* FROM mt_online_item t1, cr_temp.resp_ind_xref_ph t2 WHERE t1.individual_id = t2.individual_id;

create table cr_temp.resp_mt_offline_ord 
AS
    SELECT t1.* FROM mt_offline_ord t1, cr_temp.resp_ind_xref_ph t2 WHERE t1.individual_id = t2.individual_id;

create table cr_temp.resp_email_response 
AS
    SELECT t1.*
    FROM warehouse.email_response t1
      , cr_temp.resp_ind_xref_ph t2
    WHERE t1.individual_id = t2.individual_id
        AND t1.list_id NOT IN ('10242','11367','11368')
        AND t1.keycd IS NOT NULL;

create table cr_temp.mt_resp_summary_temp
as
select
  tc.individual_id,
  tc.ind_urn,
...
from
    cr_temp.resp_ind_xref_ph tc,
    cr_temp.resp_ph ph,
    mt_abq_response abq,
    mt_adv_response adv,
    mt_fr_donation fd,
    cr_temp.resp_mt_offline_ord ofo,
    mt_fr_pledge fp,
    (select
       ph.individual_id,
       ph.keycode,
       ...
       er.bounced,
       er.maint_dt
     from cr_temp.resp_ind_xref_ph tc,
       cr_temp.resp_ph ph,
       cr_temp.resp_email_response er,
    ...
    (select
       ...
       oli.stat_cd
     from
       cr_temp.resp_mt_online_ord olo,
       cr_temp.resp_mt_online_item oli
     where
...
select
  ...
  subs_resp_cd,
  crsch_resp_cd
  from middle_tier.cr_temp.mt_resp_summary_temp;
