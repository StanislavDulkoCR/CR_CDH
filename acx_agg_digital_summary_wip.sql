...
SELECT
  ...
  to_number(SUBSTRING(max(CASE WHEN oli.mag_cd = 'CRMG'
                            THEN nvl(to_char(oli.crt_dt,'YYYYMMDD'),'00010101') ||
                                 oli.tot_gross_amt
                            ELSE null
                       END),9)) crmg_lst_sub_amt,
  to_number(SUBSTRING(max(CASE WHEN oli.mag_cd = 'CRO'
                            THEN nvl(to_char(oli.crt_dt,'YYYYMMDD'),'00010101') ||
                                 oli.tot_gross_amt
                            ELSE null
...
FROM
    ( select
     ...
     cr_temp.ind_xref ix,
     cr_temp.digital_external_ref_temp er,
     prod.agg_digital_order olo,
     prod.agg_digital_item oli,
     mt_onl_sum_temp1 t1,
     (select 
     ,sku_num             as SKU_NUM         
    ...
    from etl_proc.lookup_magazine) sku,
     (SELECT individual_id,
             SUBSTRING(max(CASE WHEN mag_cd = 'CRMG'
                             THEN nvl(to_char(end_dt,'YYYYMMDD'),'0000000') || acct_id
                             ELSE NULL
                        END),9) crmg_comb_acct_id,
             SUBSTRING(max(CASE WHEN mag_cd = 'CRO'
