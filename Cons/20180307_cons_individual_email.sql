...
drop table if exists    agg.cons_individual_email;
create table            agg.cons_individual_email
    DISTSTYLE KEY
    DISTKEY (individual_id)
    SORTKEY (individual_id)
    as
WITH cte_precons_upd /*Custom pre-Cons update goes here*/ AS (
...
FROM prod.individual_email ie
    inner join prod.id_xref idx
        on (ie.individual_id = idx.current_id or ie.individual_id = idx.former_id) and idx.id_type = 'I'
...
with cte_former_match as (
    ...
    select
        idx.current_id
        , idx.former_id
        , idx.create_feed_instance_id
        , idx.update_feed_instance_id
...
WHERE ie.email_address is not null
        /*#TODO: RMV*/ and cfm.create_feed_instance_id = 'SD-CUSTOM-CONS-TEST-INDEMA'
;
...
INSERT INTO prod.individual_email
    SELECT
        *
    FROM agg.cons_individual_email;
