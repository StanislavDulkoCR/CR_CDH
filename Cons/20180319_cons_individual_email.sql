...
* Module Name:  prod.individual_email.sql
* Date       :  2018/03/19
* Dev        :  Stanislav Dulko
...
Run only once for QA:
    ...
    CREATE TABLE prod.etalon_individual_email as SELECT * FROM prod.individual_email;

Run inbetween runs for QA:

    truncate table prod.individual_email;
    insert into prod.individual_email select * from prod.etalon_individual_email;

    drop table if exists audit.cons_individual_email;
...
create table            agg.cons_individual_email_temp
 ...
 with cte_former_match as (
    ...
    , cfm.current_id
FROM prod.individual_email main
    ...
    SELECT
        ...
        , greatest(upd.create_date, upd.update_date)             AS default_row_05
    ...
    , listagg( distinct cse.individual_id,',') WITHIN GROUP (order by cse.individual_id = cse.current_id desc) over ( partition by current_id, email_subtype ) as cons_list
...
analyze agg.cons_individual_email_temp;
 ...
 CREATE TABLE IF NOT EXISTS audit.cons_individual_email (
     ...
     individual_id BIGINT NOT NULL  ENCODE delta ,email_subtype VARCHAR(64) NOT NULL  ENCODE lzo ,email_address VARCHAR(255)   ENCODE lzo ,data_source VARCHAR(55) NOT NULL  ENCODE lzo ,acx_dma_email_flag CHAR(1)   ENCODE lzo ,acx_valid_email_mailability_flag VARCHAR(13)   ENCODE lzo ,acx_valid_email_flag CHAR(1)   ENCODE lzo ,valid_flag CHAR(1)   ENCODE lzo ,source_deliverability_indicator VARCHAR(13)   ENCODE lzo ,source_valid_flag VARCHAR(10)   ENCODE lzo ,email_client_app VARCHAR(55)   ENCODE lzo ,first_date TIMESTAMP WITHOUT TIME ZONE   ENCODE lzo ,eff_date TIMESTAMP WITHOUT TIME ZONE   ENCODE lzo ,end_date TIMESTAMP WITHOUT TIME ZONE   ENCODE lzo ,individual_email_match_hash VARCHAR(64)   ENCODE lzo ,create_date TIMESTAMP WITHOUT TIME ZONE NOT NULL  ENCODE lzo ,create_feed_instance_id VARCHAR(64) NOT NULL  ENCODE runlength ,update_date TIMESTAMP WITHOUT TIME ZONE   ENCODE lzo ,update_feed_instance_id VARCHAR(64)   ENCODE lzo ,historical_record_indicator CHAR(1)   ENCODE lzo

     /*Original Distyle/Sortkey below*/
 )
     DISTSTYLE KEY DISTKEY (individual_id) INTERLEAVED SORTKEY ( individual_id , email_subtype , email_address , data_source )
 ;
