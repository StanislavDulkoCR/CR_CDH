...
* Module Name:  mt_online_summary.sql
* Date:         4/13/2006
***************************************************************************/

CREATE TABLE mt_onl_sum_temp1
AS
SELECT
  individual_id,
  acct_id,
  ...
  case when max(max(cro_actv)) over (partition by individual_id) = 'Y'
       then first_value(acct_id) over (partition by individual_id
                                           order by max(cro_actv) desc nulls last
                                                  , to_date(substr(min(case when mag_cd = 'CRO'
                                                                       ...
                                                                       end),50),'YYYYMMDDHH24MISS'))
       else first_value(acct_id) over (partition by individual_id
                                           order by max(cro_actv) desc nulls last
                                                  , to_date(substr(min(case when mag_cd = 'CRO'
                                                                             and nvl(set_cd, 'Z') not in ('B','D')
                                                                             and nvl(to_char(strt_dt,'YYYYMMDDHH24MISS'),'00000000000000')
                                                                       ...
                                                                       end),50),'YYYYMMDDHH24MISS') desc nulls last)
...
FROM  
  (SELECT
     ...
     first_value(crmg_brk_dt) OVER (PARTITION BY t2.individual_id
                                        ORDER BY crmg_brk_dt desc nulls last) last_crmg_brk_dt,
     ...
     first_value(cro_brk_dt ) OVER (PARTITION BY t2.individual_id
                                        ORDER BY cro_brk_dt desc nulls last) last_cro_brk_dt,
     ...
     first_value(cro_acct_brk_dt ) OVER (PARTITION BY t2.individual_id, t2.acct_id
                                        ORDER BY cro_acct_brk_dt desc nulls last) last_cro_acct_brk_dt,
   ...
   FROM
     (SELECT
        ...
        CASE WHEN t1.mag_cd = 'CRMG'
              ...
              AND substr(nvl(t1.ext_keycd, nvl(t1.int_keycd, 'ZZ')),1,2) != 'WF'
      ...
      FROM mt_online_item t1
         , mt_online_ord olo
     WHERE t1.acct_id = olo.acct_id (+)
       and t1.ord_id = olo.ord_id (+)) t2)
GROUP BY individual_id, acct_id
;

CREATE TABLE mt_online_summary
AS
SELECT
  ix.individual_id,
  ix.ind_urn,
...
FROM
    ( select
              ...
              select
                individual_id
              from warehouse.individual_xographic
     ...
     ind_xref ix,
     (select distinct individual_id, internal_key from warehouse.external_ref) er,
     mt_online_ord olo,
     mt_online_item oli,
     mt_onl_sum_temp1 t1,
     ...
     warehouse.individual_xographic ixo,
     (select individual_id,
...
GROUP BY ix.individual_id, ix.ind_urn, ix.hh_id, ix.osl_hh_id
;
